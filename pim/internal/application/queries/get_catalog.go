...
func (h GetCatalogHandler) GetCatalog(ctx context.Context, query GetCatalog) ([]*domain.CatalogProduct, int64, error) {
	if query.Page < 1 {
		query.Page = 1
	}
	if query.PageSize < 1 {
	...
	return h.catalog.GetCatalog(ctx, query.UserSellerID, query.Page, query.PageSize, query.SortBy, query.SortOrder)
}
