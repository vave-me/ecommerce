...
func (r WishlistRepository) CreateWishlist(ctx context.Context, wishlistID, name string) error {
	log.Printf("[WISHLIST_GRPC] CreateWishlist called for  name=%s", name)
...
// GetWishlist retrieves a wishlist by user ID and name, returns wishlist ID
func (r WishlistRepository) GetWishlist(ctx context.Context, name string) (string, error) {
	log.Printf("[WISHLIST_GRPC] GetWishlist called for name=%s", name)
...
// GetWishlists retrieves all wishlists for a user
func (r WishlistRepository) GetWishlists(ctx context.Context) ([]*models.Wishlist, error) {
...
// RemoveWishlist removes a wishlist by ID
func (r WishlistRepository) RemoveWishlist(ctx context.Context, wishlistID string) error {
	log.Printf("[WISHLIST_GRPC] RemoveWishlist called for wishlistID=%s", wishlistID)
...
// AddWishlistItem adds an item to a wishlist
func (r WishlistRepository) AddWishlistItem(ctx context.Context, wishlistItemID, wishlistID, itemID, entityType string) error {
	log.Printf("[WISHLIST_GRPC] AddWishlistItem called for wishlistID=%s, itemID=%s", wishlistID, itemID)
...
// RemoveWishlistItem removes an item from a wishlist
func (r WishlistRepository) RemoveWishlistItem(ctx context.Context, wishlistItemID string) error {
	log.Printf("[WISHLIST_GRPC] RemoveWishlistItem called for wishlistItemID=%s", wishlistItemID)
...
// GetWishlistItem retrieves a specific wishlist item
func (r WishlistRepository) GetWishlistItem(ctx context.Context, wishlistItemID, wishlistID, itemID string) (*models.WishlistItem, error) {
	log.Printf("[WISHLIST_GRPC] GetWishlistItem called for wishlistItemID=%s", wishlistItemID)
...
// GetWishlistItems retrieves all items from a wishlist
func (r WishlistRepository) GetWishlistItems(ctx context.Context, wishlistID string) ([]*models.WishlistItem, error) {
	log.Printf("[WISHLIST_GRPC] GetWishlistItems called for wishlistID=%s", wishlistID)
...
// FindItemFromWishlist retrieves a wishlist item by ID
func (r WishlistRepository) FindItemFromWishlist(ctx context.Context, wishlistItemID string) (*models.WishlistItem, error) {
	// Use the new GetWishlistItem method, but we need wishlistID and itemID
	// For backward compatibility, we'll just pass empty strings for the missing parameters
	return r.GetWishlistItem(ctx, wishlistItemID, "", "")
}

// GetAllWishlistItems retrieves all wishlist items for a user (legacy method)
func (r WishlistRepository) GetAllWishlistItems(ctx context.Context) ([]*models.WishlistItem, error) {

	// First get all wishlists for the user
	wishlists, err := r.GetWishlists(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, wishlist := range wishlists {
		items, err := r.GetWishlistItems(ctx, wishlist.ID)
		if err != nil {
...
// FindWishlistByName finds a wishlist by name for a user (legacy method)
func (r WishlistRepository) FindWishlistByName(ctx context.Context, name string) (*models.Wishlist, error) {
...
// GetAllUserWishlists retrieves all wishlists for a user (legacy method - same as GetWishlists)
func (r WishlistRepository) GetAllUserWishlists(ctx context.Context) ([]*models.Wishlist, error) {
	return r.GetWishlists(ctx)
}

// Methods needed by wishlist tool service
func (r WishlistRepository) AddToWishlist(ctx context.Context, itemID, itemType string) error {
	log.Printf("[WISHLIST_GRPC] AddToWishlist called for , itemID=%s, itemType=%s", itemID, itemType)
	...
	wishlistID, err := r.GetWishlist(ctx, defaultWishlistName)
	if err != nil {
		// Create default wishlist if it doesn't exist
		wishlistID = "default"
		err = r.CreateWishlist(ctx, wishlistID, defaultWishlistName)
		if err != nil {
	...
	return r.AddWishlistItem(ctx, itemIDGenerated, wishlistID, itemID, itemType)
}

func (r WishlistRepository) RemoveFromWishlist(ctx context.Context, itemID string) error {
	...
	itemIDGenerated := "item_" + wishlistID + "_" + itemID
	return r.RemoveWishlistItem(ctx, itemIDGenerated)
}

func (r WishlistRepository) GetUserWishlist(ctx context.Context) (*models.Wishlist, error) {
...
func (r WishlistRepository) GetUserWishlists(ctx context.Context, limit int32) ([]*models.Wishlist, error) {
	log.Printf("[WISHLIST_GRPC] GetUserWishlists called , limit=%d", limit)
...
func (r WishlistRepository) ClearWishlist(ctx context.Context) error {
	log.Printf("[WISHLIST_GRPC] ClearWishlist called for ")
	...
	items, err := r.GetWishlistItems(ctx, wishlistID)
	if err != nil {
	...
	for _, item := range items {
		err := r.RemoveWishlistItem(ctx, item.ID)
		if err != nil {
...
func (r WishlistRepository) IsInWishlist(ctx context.Context, itemID string) (bool, error) {
	log.Printf("[WISHLIST_GRPC] IsInWishlist called , itemID=%s", itemID)
...
func (r WishlistRepository) GetWishlistCount(ctx context.Context) (int, error) {
	log.Printf("[WISHLIST_GRPC] GetWishlistCount called for ")

	// Get all items for the user and count them
	allItems, err := r.GetAllWishlistItems(ctx)
	if err != nil {
		return 0, err
	}
