...
func (p *LLMProcessor) ProcessRequest(ctx context.Context, request string, manager *domain.Manager) (*ProcessResult, error) {
	if request == "" {
	...
	maxTok := int(manager.MaxTokens)
	tools := p.toolRegistry.GetToolDefinitions()

	// For managers with tools, encourage tool usage
	toolChoice := "auto"
	if len(tools) > 0 && manager.Type != "scheduler" {
	...
	if len(message.ToolCalls) > 0 {
		return p.processWithTools(ctx, response, manager, startTime)
	}
...
func (p *LLMProcessor) processWithTools(ctx context.Context, response *ai2.CompletionResponse, manager *domain.Manager, startTime time.Time) (*ProcessResult, error) {
	if len(response.Choices) == 0 {
		return nil, fmt.Errorf("no choices in AI response")
	}
	...
	if len(finalMessage.ToolCalls) > 0 {
		return p.processWithTools(ctx, finalResponse, manager, startTime)
	}
...
func (p *LLMProcessor) ProcessWithHistory(ctx context.Context, manager *domain.Manager, currentMessage string, history []domain.ConversationMessage, contextData map[string]interface{}) (string, []domain.ManagerAction, float64, error) {
	...
	result, err := p.ProcessRequest(ctx, currentMessage, manager)
	if err != nil {
		return "", nil, 0.0, err
	}
...
// ShouldUseAI determines if AI should be used for the manager
func (p *LLMProcessor) ShouldUseAI(manager interface{}) bool {
...
func (p *LLMProcessor) ProcessStreamingRequest(ctx context.Context, request string, manager *domain.Manager, streamChan chan<- StreamChunk) error {
	defer close(streamChan)

	// For now, we'll use non-streaming and simulate streaming
	result, err := p.ProcessRequest(ctx, request, manager)
	if err != nil {
