...
func (r ReadConversationRepository) GetConversation(ctx context.Context, id string, userID string) (*domain.ReadConversation, error) {
	const query = `
		SELECT 
			id, user_id, manager_id, created_at, updated_at, 
			active, context
		FROM %s
	...
	err := row.Scan(
		&c.ID, &c.UserID, &c.ManagerID, &c.CreatedAt, &c.UpdatedAt,
		&c.Active, &c.Context,
	)
	if err != nil {
...
// GetManagerConversations retrieves conversations for a specific manager
func (r ReadConversationRepository) GetManagerConversations(ctx context.Context, managerID string, limit, offset int) ([]*domain.ReadConversation, int, error) {
	// Get total count
	const countQuery = "SELECT COUNT(*) FROM %s WHERE manager_id = $1"
	var totalCount int
	err := r.db.QueryRowContext(ctx, r.table(countQuery), managerID).Scan(&totalCount)
	if err != nil {
		return nil, 0, errors.Wrap(err, "failed to get manager conversation count")
	}
	...
	rows, err := r.db.QueryContext(ctx, r.table(query), managerID, limit, offset)
	if err != nil {
		return nil, 0, errors.Wrap(err, "failed to get manager conversations")
	}
	defer rows.Close()
...
func (r ReadConversationRepository) AddConversation(ctx context.Context, id, userID, managerID string, createdAt time.Time, context map[string]interface{}) error {
	const query = `
		INSERT INTO %s (id, user_id, manager_id, created_at, updated_at, active, context)
	...
	_, err = r.db.ExecContext(ctx, r.table(query),
		id, userID, managerID, createdAt, createdAt, true, contextJSON,
	)
	if err != nil {
