...
import (
	...
	"middleman/reviews/internal/grpc"
	"middleman/reviews/internal/handlers"
	"middleman/reviews/internal/postgres"
	...
	"middleman/reviews/internal/rest"
)
...
func Root(ctx context.Context, svc system.Service) (err error) {
	...
	container.AddScoped(constants.ReviewsRepoKey, func(c di.Container) (any, error) {
		return es.NewAggregateRepository[*domain.Review](
			domain.ReviewAggregate,
			c.Get(constants.RegistryKey).(registry.Registry),
	...
	container.AddScoped(constants.ApplicationKey, func(c di.Container) (any, error) {
		return application.New(
			c.Get(constants.ReviewsRepoKey).(domain.ReviewRepository),
	...
	//if err = reviewspb.RegisterAsyncAPI(svc.Mux()); err != nil {
	//	return err
	//}
...
func startOutboxProcessor(ctx context.Context, outboxProcessor tm.OutboxProcessor, logger zerolog.Logger) {
	go func() {
		...
		if err != nil {
			logger.Error().Err(err).Msg("reviews outbox processor encountered an error")
		}
	}()
}
