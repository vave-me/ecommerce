...
// MobileLoginWithGoogle command for handling Google OAuth authentication
type MobileLoginWithGoogle struct {
...
type MobileLoginWithGoogleHandler struct {
	users     domain.UserRepository
	auth      auth.Authenticator
	publisher ddd.EventPublisher[ddd.Event]
}

func NewMobileLoginWithGoogleHandler(
	middleman domain.MiddlemanRepository,
	users domain.UserRepository,
	auth auth.Authenticator,
	publisher ddd.EventPublisher[ddd.Event],
) MobileLoginWithGoogleHandler {
	return MobileLoginWithGoogleHandler{
		users:     users,
		auth:      auth,
		publisher: publisher,
	}
...
func (h MobileLoginWithGoogleHandler) MobileLoginWithGoogle(ctx context.Context, cmd MobileLoginWithGoogle) (string, string, string, error) {
	// Validate inputs
	if cmd.UserID == "" {
