...
func (h middlemanHandlers[T]) onCommentAdded(ctx context.Context, event ddd.Event) error {
	payload := event.Payload().(*domain.Comment)
...
//func (h middlemanHandlers[T]) onCommentEdited(ctx context.Context, event ddd.Event) error {
//	payload := event.Payload().(*domain.Comment)
