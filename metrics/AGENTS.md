# AGENTS Guide: metrics

## Business Purpose
Aggregates and serves platform operational/engagement metrics.

## Service Runtime Shape
- Entry point: `sfx_markt/metrics/cmd/service/main.go`
- Module composition/wiring: `sfx_markt/metrics/module.go`
- Codegen entrypoint: `sfx_markt/metrics/generate.go`
- Application facade: `sfx_markt/metrics/internal/application/application.go`
- gRPC server implementation: `sfx_markt/metrics/internal/grpc/server.go`
- gRPC transaction wrapper: `sfx_markt/metrics/internal/grpc/server_transaction.go`
- API contract (proto): `sfx_markt/metrics/metricspb/api.proto`
- Event contract (proto): `(not found)`
- Message contract (proto): `(not found)`

## Concrete Files To Read First
### Proto contracts
- `sfx_markt/metrics/metricspb/api.proto`

### gRPC transport layer files
- `sfx_markt/metrics/internal/grpc/server.go`
- `sfx_markt/metrics/internal/grpc/server_transaction.go`
...
- `sfx_markt/metrics/internal/handlers/integration_events.go`
- `sfx_markt/metrics/internal/handlers/integration_events_transaction.go`

### Postgres/repository files
- `sfx_markt/metrics/internal/postgres/item_metric_cache_repository.go`
- `sfx_markt/metrics/internal/postgres/user_metric_cache_repository.go`

### SQL migrations
- `sfx_markt/metrics/migrations/001_create_tables.sql`
...
- `GetItemMetric`
- `GetItemsMetric`
- `GetHighestMetricsByType`
- `GetLowestMetricsByType`
...
1. Run tests for this service:
   - `cd <workspace-root>/sfx_markt && go test ./metrics/...`
2. Start the service directly:
   - `cd <workspace-root>/sfx_markt && go run ./metrics/cmd/service`
3. Optional container run (if profile exists):
   - `cd <workspace-root>/sfx_markt && docker compose --env-file ./docker/.env --profile metrics up`
...
2. Regenerate this service:
   - `cd <workspace-root>/sfx_markt/metrics && go generate ./...`
3. If only proto files changed:
   - `cd <workspace-root>/sfx_markt/metrics && buf generate`
4. Re-run tests:
   - `cd <workspace-root>/sfx_markt && go test ./metrics/...`

## Add or Remove a Command (Detailed)
1. Update command request/response messages and RPC (if externally callable) in `sfx_markt/metrics/metricspb/api.proto`.
...
1. Add query request/response messages + RPC in `sfx_markt/metrics/metricspb/api.proto`.
