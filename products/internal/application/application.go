...
type (
	...
	Queries interface {
		...
		GetProductBySKU(ctx context.Context, query queries.GetProductBySKU) (*domain.CatalogProduct, error)
		GetProductsBySKUs(ctx context.Context, query queries.GetProductsBySKUs) ([]*domain.CatalogProduct, []string, error)
		GetVariant(ctx context.Context, query queries.GetVariant) (*domain.CatalogVariant, error)
		GetVariants(ctx context.Context, query queries.GetVariants) ([]*domain.CatalogVariant, int64, error)
	}
	...
	appQueries struct {
		...
		queries.GetProductBySKUHandler
		queries.GetProductsBySKUsHandler
		queries.GetVariantHandler
		queries.GetVariantsHandler
	}
)
...
func New(products domain.ProductRepository,
	...
	return &Application{
		...
		appQueries: appQueries{
			...
			GetProductBySKUHandler:           queries.NewGetProductBySKUHandler(catalog),
			GetProductsBySKUsHandler:         queries.NewGetProductsBySKUsHandler(catalog),
			GetVariantHandler:                queries.NewGetVariantHandler(variantCatalog),
			GetVariantsHandler:               queries.NewGetVariantsHandler(variantCatalog),
		},
	}
}
