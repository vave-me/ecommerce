...
/**
 * Add a new ERP connector (v2 endpoint)
 * @param {Object} data - Connector registration data
 * @param {string} data.name - Connector name
 * @param {string} data.type - Connector type (odoo, dynamics365, netsuite, sap, erpnext, frappe)
 * @param {string} data.environment - Environment (production, staging, development, sandbox)
 * @param {string} data.baseUrl - Base URL for the ERP system
 * @param {string} data.authType - Authentication type (basic, oauth2, oauth1, apikey, token)
 * @param {Object} data.authConfig - Authentication configuration
 * @param {boolean} [data.webhookEnabled=false] - Whether webhooks are enabled
 * @param {boolean} [data.syncEnabled=true] - Whether sync is enabled
 * @param {number} [data.syncIntervalSeconds=300] - Sync interval in seconds
 * @param {number} [data.batchSize=100] - Batch size for sync operations
 * @returns {Promise<Object>} Created connector response
 */
export const addConnector = async (data) => {
  try {
    const response = await axiosInstance.post('/erp/connectors/v2', data);
    return response.data;
  } catch (error) {
    // Error: 'Error adding ERP connector:', error...
    throw error;
  }
};

/**
 * Update an existing ERP connector
 * @param {string} connectorId - Connector ID
 * @param {Object} data - Update data (same fields as addConnector)
 * @returns {Promise<Object>} Updated connector
 */
export const updateConnector = async (connectorId, data) => {
  try {
    const response = await axiosInstance.put(`/erp/connectors/${connectorId}`, data);
    return response.data;
  } catch (error) {
    // Error: 'Error updating ERP connector:', error...
    throw error;
  }
};

/**
 * Remove an ERP connector
 * @param {string} connectorId - Connector ID
 * @param {boolean} [force=false] - Force removal even with active syncs
 * @returns {Promise<Object>} Removal result
 */
export const removeConnector = async (connectorId, force = false) => {
  try {
    const response = await axiosInstance.delete(`/erp/connectors/${connectorId}`, {
      params: { force }
    });
    return response.data;
  } catch (error) {
    // Error: 'Error removing ERP connector:', error...
    throw error;
  }
};

/**
 * Toggle connector activation status
 * @param {string} connectorId - Connector ID
 * @param {Object} data - Toggle data
 * @param {boolean} data.activate - Whether to activate or deactivate
 * @param {string} [data.reason] - Reason for toggle
 * @returns {Promise<Object>} Toggle result
 */
export const toggleConnector = async (connectorId, data) => {
  try {
    const response = await axiosInstance.post(`/erp/connectors/${connectorId}/toggle`, data);
    return response.data;
  } catch (error) {
    // Error: 'Error toggling connector:', error...
    throw error;
  }
};
