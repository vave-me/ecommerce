...
export async function generateMetadata(props) {
    ...
    try {
        ...
        t = await getTranslations({locale, namespace: 'DashboardDetailPage'}); // Changed from PostDetailPage to DashboardDetailPage
        post = await getPost(slug); // Use getPost instead of getPostsBySlug
        if (!post) {
        ...
        return {
            ...
            alternates: {
                canonical: `https://www.sfx-market.de/${locale}/dashboard/${params.category}/${slug}`,
            },
        };
    } catch (err) {
        // Fallback metadata on error
        const errorTitle = t ? t('metaTitleError') : "Error Loading Dashboard Post";
...
export default async function DashboardDetailPage(props) {
    ...
    try {
        ...
        let postData = null;
        try {
            // Fetch the specific post data using the slug (which is the post ID)
        ...
        const articleJsonLd = {
            ...
            'mainEntityOfPage': {
                '@type': 'WebPage',
                '@id': `https://www.sfx-market.de/${locale}/dashboard/${category}/${slug}` // Changed from /posts/ to /dashboard/
            }
        };
        ...
        let errorTitle = "Error Loading Dashboard Post";
        let errorMessage = "Could not load the dashboard post details. Please try again later.";
        try {
            if (!t) t = await getTranslations({locale: props.params?.locale || 'en', namespace: "DashboardDetailPage"});
