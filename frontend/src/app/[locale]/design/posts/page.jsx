...
const ImprovedPostCard = ({post}) => {
    ...
    // Memoized computed values from real data
    const computedData = useMemo(() => {
        return {
            ...
            location: actualPost?.lat && actualPost?.lng && (actualPost.lat !== 0 || actualPost.lng !== 0)
                ...
                : null,
            // Handle images - will be populated from media fetch
            images: mediaItems.length > 0 ? mediaItems.map(item => item.url) : (actualPost?.thumbnail ? [actualPost.thumbnail] : []),
    ...
    // Fetch media data when post changes
    useEffect(() => {
        const fetchMedia = async () => {
            ...
            try {
                ...
                const mediaResponse = await getMediaByItem(actualPost.id);
                if (mediaResponse?.media?.mediaOrder?.length > 0) {
                    const formattedMedia = mediaResponse.media.mediaOrder.map(item => ({
                        ...
                        alt: item.altText || actualPost.name || 'Post media'
                    }));
                    setMediaItems(formattedMedia);
                } else {
                    // Fallback to thumbnail if no media gallery
                    if (actualPost?.thumbnail) {
                        setMediaItems([{
                            id: 'thumbnail',
                            url: actualPost.thumbnail,
                            type: 'image',
                            alt: actualPost.name || 'Post thumbnail'
                        }]);
                    }
                }
            } catch (error) {
        ...
        fetchMedia();
    }, [actualPost?.id, actualPost?.thumbnail, actualPost?.name]);
    // Merge actual fields from the API with author data
    const postData = {
        ...
        isLiked: false, // TODO: Get from user's activity data
        isBookmarked: false, // TODO: Get from user's bookmarks
        isReposted: false, // TODO: Get from user's reposts
        isFavorite: false, // TODO: Get from user's favorites
        isPinned: false, // TODO: Get from post metadata
        isPromoted: false, // TODO: Get from post metadata
        // Timestamps
        ...
        <div className={styles.articleContainer}>
            ...
            <div className={styles.card}>
                <div className={styles.cardContent}>
                    ...
                    {!isLoadingMedia && postData.images && postData.images.length > 0 && (
                        <div className={styles.mediaContainer}>
                            ...
                            <div className={styles.imageWrapper}>
                                {mediaItems[showImageIndex]?.type === 'video' ? (
                                    <video
                                    ...
                                    <Image
                                        src={postData.images[showImageIndex]}
                                        alt={mediaItems[showImageIndex]?.alt || "Post attachment"}
                                        className={styles.postImage}
                                        width={800}
                                        height={450}
                    ...
                    {isLoadingMedia && (
                        <div className={styles.mediaContainer}>
                            <div className={styles.mediaLoading}>
                                <div className={styles.mediaLoadingSkeleton}></div>
                            </div>
                        </div>
                    )}
                    {/* Hashtags */}
