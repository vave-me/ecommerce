...
export async function generateMetadata(props) {
    ...
    try {
        ...
        t = await getTranslations({locale, namespace: 'PostsPage'});
        ...
        return {
            ...
            alternates: {
                canonical: `https://www.sfx-market.de/${locale}/posts/${category}`,
            },
        };
    } catch (e) {
        // Fallback metadata if anything fails
        const fallbackCategoryName = category || 'Category'; // Use category if available
        const fallbackTitle = t ? t('metaTitleCategoryFallback', {categoryName: fallbackCategoryName}) : `${fallbackCategoryName} Posts`;
        const fallbackDescription = t ? t('metaDescriptionCategoryFallback', {categoryName: fallbackCategoryName}) : `Read the latest posts in the ${fallbackCategoryName} category.`;
        return {
            title: fallbackTitle,
...
export default async function PostsCategoryPage(props) {
    ...
    try {
        ...
        const postsJsonLd = {
            ...
            itemListElement: serverPosts.map((post, index) => ({
                ...
                url: `https://www.sfx-market.de/${locale}/posts/${category}/${post.slug || post.id}`, // Dynamic URL
                item: { // Embed BlogPosting data
                    ...
                    'mainEntityOfPage': {
                        '@type': 'WebPage',
                        '@id': `https://www.sfx-market.de/${locale}/posts/${category}/${post.slug || post.id}`
                    }
                }
            })),
        };
        ...
        let errorTitle = "Error Loading Category Posts";
        let errorMessage = "Could not load posts for this category. Please try again later.";
        try {
            if (!t) t = await getTranslations({locale: props.params?.locale || 'en', namespace: "PostsPage"});
