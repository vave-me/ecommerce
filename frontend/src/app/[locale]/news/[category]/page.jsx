...
export async function generateMetadata(props) {
    ...
    try {
        ...
        return {
            ...
            alternates: {
                canonical: `https://www.sfx-market.de/${locale}/news/${category}`,
            },
        };
    } catch (e) {
        // Fallback metadata if anything fails
        const fallbackCategoryName = category || 'Category'; // Use category if available
        const fallbackTitle = t ? t('metaTitleCategoryFallback', {categoryName: fallbackCategoryName}) : `${fallbackCategoryName} News`;
        const fallbackDescription = t ? t('metaDescriptionCategoryFallback', {categoryName: fallbackCategoryName}) : `Read the latest news in the ${fallbackCategoryName} category.`;
        return {
            title: fallbackTitle,
...
export default async function NewsCategoryPage(props) {
    ...
    try {
        ...
        /* 4) Fetch news posts filtered by category */
        ...
        try {
            ...
            fetchError = t("errorFetchingNewsCategory", {category: categoryName});
        }
        ...
        const postsJsonLd = {
            ...
            itemListElement: serverPosts.map((post, index) => ({
                ...
                url: `https://www.sfx-market.de/${locale}/news/${category}/${post.slug || post.id}`, // Use /news/ URLs
                item: { // Embed BlogPosting data
                    '@type': 'BlogPosting',
                    'headline': post.title || post.name || t('newsCard.unnamedNews'),
                    'image': post.thumbnail || post.image || '',
                    'description': post.description || post.summary || '',
                    'author': {
                        '@type': 'Person',
                        'name': post.author?.name || t('newsCard.defaultAuthor')
                    },
                    'publisher': {
                        '@type': 'Organization',
                    ...
                    'mainEntityOfPage': {
                        '@type': 'WebPage',
                        '@id': `https://www.sfx-market.de/${locale}/news/${category}/${post.slug || post.id}`
                    }
                }
            })),
        };
        ...
        let errorTitle = "Error Loading Category News";
        let errorMessage = "Could not load news for this category. Please try again later.";
        try {
