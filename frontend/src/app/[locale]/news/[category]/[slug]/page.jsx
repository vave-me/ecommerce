...
import { getPost } from "../../../../../api/searchApi"; // Already using correct API
...
export async function generateMetadata(props) {
    ...
    try {
        ...
        t = await getTranslations({ locale, namespace: 'NewsDetailPage' }); // Use NewsDetailPage namespace
        ...
        return {
            ...
            alternates: {
                canonical: `https://www.sfx-market.de/${locale}/news/${params.category}/${slug}`,
            },
        };
    } catch (err) {
        // Fallback metadata on error
        const errorTitle = t ? t('metaTitleError') : "Error Loading News";
...
export default async function NewsDetailPage(props) {
    ...
    try {
        ...
        const articleJsonLd = {
            ...
            '@type': 'BlogPosting',
            'headline': postData.title || postData.name || t('newsCard.unnamedNews'), // Use NewsCard fallback key
            ...
            'author': {
                '@type': 'Person',
                'name': postData.author?.name || t('newsCard.defaultAuthor') // Use NewsCard fallback key
            },
            'publisher': {
                '@type': 'Organization',
            ...
            'mainEntityOfPage': {
                '@type': 'WebPage',
                '@id': `https://www.sfx-market.de/${locale}/news/${category}/${slug}` // Use /news/ URLs
            }
        };
        ...
        let errorMessage = "Could not load the news details. Please try again later.";
        try {
            if (!t) t = await getTranslations({ locale: props.params?.locale || 'en', namespace: "NewsDetailPage" });
