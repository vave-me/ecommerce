...
export default function LoginHookForm() {
    const t = useTranslations("LoginForm");       // ← next-intl hook
    ...
    /* 1) Yup schema that uses translated messages -------------- */
    const schema = useMemo(() => Yup.object({
        ...
        confirmPassword: isSignUp
            ? Yup.string()
                ...
                .required(t("errors.required"))
            : Yup.string(),
        firstName: isSignUp ? Yup.string().required(t("errors.required")) : Yup.string(),
        lastName: isSignUp ? Yup.string().required(t("errors.required")) : Yup.string(),
        userName: isSignUp ? Yup.string().required(t("errors.required")) : Yup.string(),
        address: isSignUp ? Yup.string().required(t("errors.required")) : Yup.string()
    }), [isSignUp, t]);
    /* 2) React-Hook-Form --------------------------------------- */
    const {
        register,
        ...
        if (val.length > 2) {
            try {
                ...
                if (result.suggestionAddress && result.suggestionAddress.length > 0) {
                    // Extract just the address strings from the suggestions
    ...
    const handleGoBack = () => {
        // If there's a redirect URL, go back to it
                    ...
                    onLoad={() => {
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                // Use ref instead of direct DOM manipulation
            ...
            <section className={styles.section}>
                <div className={styles.hero}>
                    ...
                    {isSignUp ? (
                        ...
                        <p>Access your personalized experience, connect with others, and discover opportunities tailored just for you.</p>
                    )}
                </div>
