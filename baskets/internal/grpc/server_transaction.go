...
func (s serverTx) GetTotalBasket(ctx context.Context, request *basketspb.GetTotalBasketAmountRequest) (resp *basketspb.GetTotalBasketAmountResponse, err error) {
	ctx = s.c.Scoped(ctx)
	defer func(tx *sql.Tx) {
		err = s.closeTx(tx, err)
	}(di.Get(ctx, constants.DatabaseTransactionKey).(*sql.Tx))
	...
	return next.GetTotalBasket(ctx, request)
}
func (s serverTx) GetCurrentBasket(ctx context.Context, request *basketspb.GetCurrentBasketRequest) (resp *basketspb.GetCurrentBasketResponse, err error) {
	ctx = s.c.Scoped(ctx)
	defer func(tx *sql.Tx) {
		err = s.closeTx(tx, err)
	}(di.Get(ctx, constants.DatabaseTransactionKey).(*sql.Tx))
	...
	return next.GetCurrentBasket(ctx, request)
}
