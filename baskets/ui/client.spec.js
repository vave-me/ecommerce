...
describe('Baskets UI', () => {
    ...
    context('calling startBasket', () => {
        describe('with a user customer ID', () => {
            let basketId;
            ...
            it('should start a new basket for the user customer', () => {
                return provider.executeTest((mockServer) => {
                    const client = new Client(mockServer.url);
                    return client.startBasket('user-customer-id')
                        .then((response) => {
        ...
        describe('without any user customer ID', () => {
            before(() => {
                provider.uponReceiving('a request to start a basket without a userCustomerId')
                    .withRequest({
                        method: 'POST',
                        path: '/api/baskets',
                        body: {
                    ...
                    .willRespondWith({
                        body: MatchersV3.like({
                            "message": "the user customer id cannot be blank",
                        }),
                        headers: {'Content-Type': 'application/json'},
                        status: 400,
                    });
            ...
            it('should not start a new basket for the user customer', async () => {
                return provider.executeTest((mockServer) => {
                    const client = new Client(mockServer.url);
    ...
    context('calling addItem', () => {
        ...
        describe('with a valid product and quantity', () => {
            before(() => {
                ...
                provider.given('a user seller exists')
