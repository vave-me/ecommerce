...
import (
	...
	"middleman/following"
	"middleman/following/migrations"
...
func main() {
	if err := run(); err != nil {
		fmt.Printf("following exitted abnormally: %s\n", err)
		os.Exit(1)
	}
}
...
func run() (err error) {
	...
	publicMethods := []string{
		"/followingpb.FollowingService/GetFollowing",
		"/followingpb.FollowingService/GetFollow",
	}
	...
	if err = following.Root(s.Waiter().Context(), s); err != nil {
		return err
	}

	fmt.Println("started following service")
	defer fmt.Println("stopped following service")
