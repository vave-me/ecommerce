...
import (
	...
	"middleman/wishlists"
	"middleman/wishlists/migrations"
	"net/http"
	"os"
)
...
func main() {
	if err := run(); err != nil {
		fmt.Printf("wishlists exitted abnormally: %s\n", err)
		os.Exit(1)
	}
}

// ADD THIS TO ME WIHSILST
func run() (err error) {
	...
	if err = wishlists.Root(s.Waiter().Context(), s); err != nil {
		return err
	}

	fmt.Println("started wishlists service")
	defer fmt.Println("stopped wishlists service")
