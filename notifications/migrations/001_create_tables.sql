...
CREATE INDEX is_read_alerts ON alerts (is_read);
CREATE INDEX products_cache_alerts_idx ON alerts (id);

CREATE TRIGGER created_at_alerts_trgr
    BEFORE UPDATE
    ON alerts
    FOR EACH ROW EXECUTE PROCEDURE created_at_trigger();
CREATE TRIGGER updated_at_alerts_trgr
    BEFORE UPDATE
    ON alerts
    FOR EACH ROW EXECUTE PROCEDURE updated_at_trigger();
