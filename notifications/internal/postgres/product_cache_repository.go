...
func (r ProductCacheRepository) Add(ctx context.Context, productID, userSellerID, name, description string, price int64) error {
	const query = `INSERT INTO %s (id, user_seller_id, name, description, base_price) VALUES ($1, $2, $3, $4,$5)`

	_, err := r.db.ExecContext(ctx, r.table(query), productID, userSellerID, name, description, price)
	if err != nil {
...
func (r ProductCacheRepository) UpdatePrice(ctx context.Context, productID string, delta int64) error {
	const query = `UPDATE %s SET base_price = price + $2 WHERE id = $1`
...
func (r ProductCacheRepository) Find(ctx context.Context, productID string) (*domain.Product, error) {
	const query = `SELECT user_seller_id, name,description, base_price FROM %s WHERE id = $1 LIMIT 1`

	product := &domain.Product{
		ID: productID,
	}

	err := r.db.QueryRowContext(ctx, r.table(query), productID).Scan(&product.UserSellerID, &product.Name, &product.Description, &product.BasePrice)
	if err != nil {
		...
		return product, r.Add(ctx, product.ID, product.UserSellerID, product.Name, product.Description, product.BasePrice)
	}
