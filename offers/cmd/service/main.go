...
func run(logger *logrus.Logger) error {
	...
	if err := offers.Root(s.Waiter().Context(), s); err != nil {
		logger.WithError(err).Error("Failed to initialize offers module")
		return err
	}

	logger.Info("Offers service started")
	defer logger.Info("Offers service stopped")
