...
func (o orchestrator[T]) processResult(ctx context.Context, result stepResult[T]) (err error) {
	...
	} else if !result.ctx.Done {
		// If no command was published but saga is not done, continue to next step
		// This handles the case where a step is skipped (returns nil command)
		nextResult := o.execute(ctx, result.ctx)
		if nextResult.err != nil {
			return nextResult.err
		}
		return o.processResult(ctx, nextResult)
	}
