...
func (h GetPublicCatalogHandler) GetPublicCatalog(ctx context.Context, query GetPublicCatalog) ([]*domain.CatalogPost, int64, error) {
	if query.Page < 1 {
		query.Page = 1
	}
	if query.PageSize < 1 {
	...
	return h.catalog.GetPublicCatalog(ctx, query.UserId, query.Page, query.PageSize, query.SortBy, query.SortOrder)
}
