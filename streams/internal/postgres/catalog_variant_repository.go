...
func (r CatalogVariantRepository) FindVariant(ctx context.Context, variantID string) (*domain.CatalogVariant, error) {
	...
	cv.Status = domain.ToStreamStatus(statusStr)
	cv.Condition = domain.ToStreamCondition(condStr)
...
func (r CatalogVariantRepository) GetVariant(ctx context.Context, variantID string) (*domain.CatalogVariant, error) {
	...
	return &domain.CatalogVariant{
		...
		StreamID:     catalogVariant.StreamID,
...
// -----------------------------------------------------------------------------
// 6) GetVariantsByStream
// -----------------------------------------------------------------------------
func (r CatalogVariantRepository) GetVariantsByStream(
	ctx context.Context,
	productID string,
	page, pageSize int64,
	sortBy, sortOrder string,
...
func (r CatalogVariantRepository) GetVariantsByCategory(
	...
	// or doing a JOIN with streams. We'll just return an error for now.
	return nil, 0, errors.Wrap(errors.ErrBadRequest,
		"Not Implemented: GetVariantsByCategory requires a join on the streams table or storing category_id in variants")
}
