...
func (r CatalogRepository) AddStream(
	ctx context.Context,
	id, name, description string,
	basePrice int64,
	userSellerID, categoryID, categorySlug, brand string,
	condition domain.StreamCondition,
	model string,
	tags []string,
	manageStock bool,
	stock int64,
...
func (r CatalogRepository) RebrandStream(
	ctx context.Context,
	productID string,
	newName string,
...
// -----------------------------------------------------------------------------
// 3) UpdateStream (now storing tags + attributes)
// -----------------------------------------------------------------------------
func (r CatalogRepository) UpdateStream(
	ctx context.Context, productID, name, description string,
	basePrice int64,
	...
	weight, height, width, depth int64, status domain.StreamStatus,
...
// 4) UpdatePrice
// -----------------------------------------------------------------------------
func (r CatalogRepository) UpdatePrice(ctx context.Context, productID string, oldPrice, newPrice int64) error {
	const query = `
      UPDATE %s
      SET base_price = $3,
          updated_at = NOW()
	...
	if affected == 0 {
		return errors.Wrap(err, "no product updated; either product not found or base_price mismatch")
	}
	return nil
}

// -----------------------------------------------------------------------------
func (r CatalogRepository) UpdateThumbnail(ctx context.Context, productID, thumbnail string) error {
	const query = `
      UPDATE %s
      SET thumbnail = $2,
          updated_at = NOW()
      WHERE id = $1
    `
	res, err := r.db.ExecContext(ctx, r.table(query), productID, thumbnail)
	if err != nil {
		return errors.Wrap(err, "updating thumbnail")
	}
	affected, _ := res.RowsAffected()
	if affected == 0 {
		return errors.Wrap(err, "no product updated; either product not found or thumbnail mismatch")
	}
	return nil
}

func (r CatalogRepository) GetStreamsWithFilters(
	ctx context.Context,
	name string,
	categoryID string,
	categorySlug string,
...
) ([]*domain.CatalogStream, int64, error) {
	...
	if err := r.db.QueryRowContext(ctx, countQuery, args...).Scan(&totalCount); err != nil {
		return nil, 0, errors.Wrap(err, "GetStreamsWithFilters: counting total items")
	}

	// 6) Build final SELECT for product rows
	...
	if err != nil {
		return nil, 0, errors.Wrap(err, "GetStreamsWithFilters: query rows error")
	}
	defer rows.Close()

	var results []*domain.CatalogStream
	for rows.Next() {
		cp := &domain.CatalogStream{}
		var (
			tagsStr  string
			attrsStr string
			optsStr  string
		...
		if scErr != nil {
			return nil, 0, errors.Wrap(scErr, "GetStreamsWithFilters scanning product row")
		}
	...
	if err = rows.Err(); err != nil {
		return nil, 0, errors.Wrap(err, "GetStreamsWithFilters final iteration err")
	}
...
func (r CatalogRepository) AdjustStock(
	...
	const query = `
      ...
      WHERE id = $1
        ...
        AND stock = $4
    `
	res, err := r.db.ExecContext(ctx, r.table(query), productID, userSellerID, newStock, oldStock)
	if err != nil {
		return errors.Wrap(err, "adjusting product stock")
	}
	if rows, _ := res.RowsAffected(); rows == 0 {
		return errors.Wrap(err, "no product updated; stock mismatch or not found")
	}
	return nil
}
...
func (r CatalogRepository) ToggleNegotiable(
	...
	const query = `
      ...
      WHERE id = $1
        ...
        AND negotiable = $3
    `
	res, err := r.db.ExecContext(ctx, r.table(query), productID, userSellerID, currentValue)
	if err != nil {
		return errors.Wrap(err, "toggle negotiable error")
	}
	if rows, _ := res.RowsAffected(); rows == 0 {
		return errors.Wrap(err, "no product updated; negotiable mismatch or not found")
	}
	return nil
}
...
// 7) RemoveStream
// -----------------------------------------------------------------------------
func (r CatalogRepository) RemoveStream(
	ctx context.Context,
	productID string,
	userSellerID string,
) error {
	const query = `
      ...
      WHERE id = $1
        AND user_seller_id = $2
    `
	_, err := r.db.ExecContext(ctx, r.table(query), productID, userSellerID)
	return err
}
...
// 8) MarkStreamPawned
// -----------------------------------------------------------------------------
func (r CatalogRepository) MarkStreamPawned(
	ctx context.Context,
	productID string,
	userSellerID string,
) error {
...
// 9) MarkStreamLeased
// -----------------------------------------------------------------------------
func (r CatalogRepository) MarkStreamLeased(
	ctx context.Context,
	productID string,
	userSellerID string,
) error {
...
// 10) MarkStreamSold
// -----------------------------------------------------------------------------
func (r CatalogRepository) MarkStreamSold(
	ctx context.Context,
	productID string,
	userSellerID string,
	finalPrice int64,
	...
	const query = `
      ...
      WHERE id = $1
        AND user_seller_id = $2
    `
	_, err := r.db.ExecContext(ctx, r.table(query), productID, userSellerID, finalPrice)
	return err
}
...
// 11) ArchiveStream
// -----------------------------------------------------------------------------
func (r CatalogRepository) ArchiveStream(
	ctx context.Context,
	productID string,
	userSellerID string,
) error {
...
// 12) Find
// -----------------------------------------------------------------------------
func (r CatalogRepository) Find(ctx context.Context, productID string) (*domain.CatalogStream, error) {
	const query = `
      SELECT
        name,
        description,
      ...
      LIMIT 1
    `
	row := r.db.QueryRowContext(ctx, r.table(query), productID)
	cp := &domain.CatalogStream{ID: productID}
...
// 13) GetCatalog
// -----------------------------------------------------------------------------
func (r CatalogRepository) GetCatalog(ctx context.Context, userSellerID string, page, pageSize int64, sortBy, sortOrder string) ([]*domain.CatalogStream, int64, error) {
	...
	if err := r.db.QueryRowContext(ctx, countQuery, userSellerID).Scan(&totalCount); err != nil {
		return nil, 0, errors.Wrap(err, "counting streams")
	}
	...
	var streams []*domain.CatalogStream
	for rows.Next() {
		cp := &domain.CatalogStream{}
		var (
			tagsStr  string
		...
		if err != nil {
			return nil, 0, errors.Wrap(err, "scanning product row")
		}
		cp.Tags = stringToSlice(tagsStr)
		cp.Attributes = stringToAttributes(attrsStr)
		cp.MiddlemanService = mmsBool
	...
	if err = rows.Err(); err != nil {
		return nil, 0, errors.Wrap(err, "reading final product rows")
	}

	return streams, totalCount, nil
}

// -----------------------------------------------------------------------------
// 13) GetCatalog
// -----------------------------------------------------------------------------
func (r CatalogRepository) GetPublicCatalog(ctx context.Context, userSellerID string, page, pageSize int64, sortBy, sortOrder string) ([]*domain.CatalogStream, int64, error) {
	offset := (page - 1) * pageSize
	validSortFields := map[string]bool{
		"name":       true,
		"base_price": true,
...
func (r CatalogRepository) GetStreams(ctx context.Context, page, pageSize int64, sortBy, sortOrder string) ([]*domain.CatalogStream, int64, error) {
	offset := (page - 1) * pageSize
	validSortFields := map[string]bool{
		"name":       true,
		"base_price": true,
	...
	if err != nil {
		return nil, 0, errors.Wrap(err, "querying all streams")
	}
	defer rows.Close()
	...
	if err = rows.Err(); err != nil {
		return nil, 0, errors.Wrap(err, "finalizing product rows")
	}
...
// -----------------------------------------------------------------------------
// 15) GetStreamsByCategory
// -----------------------------------------------------------------------------
func (r CatalogRepository) GetStreamsByCategory(ctx context.Context, categoryID string, page, pageSize int64, sortBy, sortOrder string) ([]*domain.CatalogStream, int64, error) {
	offset := (page - 1) * pageSize
	validSortFields := map[string]bool{
		"name":       true,
		"base_price": true,
	...
	if err := r.db.QueryRowContext(ctx, countQ, categoryID).Scan(&totalCount); err != nil {
		return nil, 0, errors.Wrap(err, "counting streams by category")
	}
	...
	if err != nil {
		return nil, 0, errors.Wrap(err, "querying streams by category")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, 0, errors.Wrap(err, "scanning product row by category")
		}
		cp.Tags = stringToSlice(tagsStr)
		cp.Attributes = stringToAttributes(attrsStr)
		cp.MiddlemanService = mmsBool
...
func (r CatalogRepository) GetStreamsByCategorySlug(ctx context.Context, categorySlug string, page, pageSize int64, sortBy, sortOrder string) ([]*domain.CatalogStream, int64, error) {
	offset := (page - 1) * pageSize
	validSortFields := map[string]bool{
		"name":       true,
		"base_price": true,
...
func (r *CatalogRepository) FindByLocation(
	...
	limit int,
) ([]*domain.CatalogStream, error) {

	// We SELECT all the columns you also retrieve in GetStreams or Find,
	...
	if err != nil {
		return nil, errors.Wrap(err, "querying streams by location")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, errors.Wrap(err, "scanning product row by location")
		}
	...
	if err = rows.Err(); err != nil {
		return nil, errors.Wrap(err, "finalizing product rows by location")
	}
...
// GetStreamBySKU retrieves a single product by its SKU
func (r CatalogRepository) GetStreamBySKU(ctx context.Context, sku string) (*domain.CatalogStream, error) {
	query := fmt.Sprintf(`
		SELECT
			id,
			name,
	...
	if err != nil {
		return nil, errors.Wrap(err, "scanning product by SKU")
	}
...
// GetStreamsBySKUs retrieves multiple streams by their SKUs
func (r CatalogRepository) GetStreamsBySKUs(ctx context.Context, skus []string) ([]*domain.CatalogStream, error) {
	if len(skus) == 0 {
		return []*domain.CatalogStream{}, nil
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "querying streams by SKUs")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, errors.Wrap(err, "scanning product row")
		}
	...
	if err = rows.Err(); err != nil {
		return nil, errors.Wrap(err, "finalizing product rows")
	}
