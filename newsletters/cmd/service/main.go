...
// UNSUBSCRIBE ME FROM NEWSLETTER
func run(logger *logrus.Logger) error {
	...
	if err := newsletters.Root(s.Waiter().Context(), s); err != nil {
		logger.WithError(err).Error("Failed to initialize newsletters module")
		return err
	}

	logger.Info("Newsletters service started")
	defer logger.Info("Newsletters service stopped")
