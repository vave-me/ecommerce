...
func (r ProductRepository) UpdateProductDetails(ctx context.Context, productID, name, description string, basePrice int64, stock int64, sku string, categoryID string, status string) error {
	log.Printf("UpdateProductDetails: updating product with ID=%s via gRPC", productID)
	...
	log.Printf("UpdateProductDetails: successfully updated product with ID=%s", productID)
	return nil
}

func (r ProductRepository) SearchProductsByName(ctx context.Context, name string) ([]*models.Product, error) {
	log.Printf("SearchProductsByName: searching products with name=%s via gRPC", name)

	// Create a timeout context with 20-second deadline
	timeoutCtx, cancel := context.WithTimeout(ctx, 20*time.Second)
	defer cancel()

	log.Printf("SearchProductsByName: created 20-second timeout context for name=%s", name)

	conn, err := r.dial(timeoutCtx)
	if err != nil {
		log.Printf("SearchProductsByName: failed to dial gRPC: %v", err)
		return nil, err
	}
	defer conn.Close()

	log.Printf("SearchProductsByName: successfully connected to gRPC for name=%s", name)

	client := productspb.NewProductsServiceClient(conn)

	log.Printf("SearchProductsByName: calling GetProductsWithFilters RPC for name=%s", name)
	...
	if err != nil {
		log.Printf("SearchProductsByName: GetProductsWithFilters RPC FAILED for name=%s: %v", name, err)
		return nil, fmt.Errorf("GetProductsWithFilters RPC failed: %w", err)
	}

	log.Printf("SearchProductsByName: GetProductsWithFilters RPC SUCCESS for name=%s, got %d products", name, len(resp.GetProducts()))
	...
	log.Printf("SearchProductsByName: returning %d products for name=%s", len(products), name)
	return products, nil
}

func (r ProductRepository) GetProductSuggestions(ctx context.Context, name string) ([]*models.Product, error) {
	// Use SearchProductsByName as a fallback for suggestions
	return r.SearchProductsByName(ctx, name)
}

func (r ProductRepository) UpdateProductThumbnail(ctx context.Context, productID string, thumbnail string) error {
	log.Printf("UpdateProductThumbnail: updating thumbnail for product ID=%s via gRPC", productID)
	...
	log.Printf("UpdateProductThumbnail: successfully updated thumbnail for product ID=%s", productID)
	return nil
}

// GetProductByID retrieves a product by ID from the products microservice (via gRPC).
func (r ProductRepository) GetProductByID(ctx context.Context, productID string) (*models.Product, error) {
	log.Printf("GetProductByID: retrieving product with ID=%s via gRPC", productID)

	// Create a timeout context with 20-second deadline
	timeoutCtx, cancel := context.WithTimeout(ctx, 20*time.Second)
	defer cancel()

	log.Printf("GetProductByID: created 20-second timeout context for productID=%s", productID)

	conn, err := r.dial(timeoutCtx)
	if err != nil {
		log.Printf("GetProductByID: failed to dial gRPC: %v", err)
		return nil, err
	}
	defer conn.Close()

	log.Printf("GetProductByID: successfully connected to gRPC for productID=%s", productID)

	client := productspb.NewProductsServiceClient(conn)

	log.Printf("GetProductByID: calling GetProduct RPC for productID=%s", productID)

	resp, err := client.GetProduct(timeoutCtx, &productspb.GetProductRequest{Id: productID})
	if err != nil {
		log.Printf("GetProductByID: GetProduct RPC FAILED for productID=%s: %v", productID, err)
		return nil, fmt.Errorf("GetProduct RPC failed: %w", err)
	}

	log.Printf("GetProductByID: GetProduct RPC SUCCESS for productID=%s", productID)
	return r.productToDomain(resp.GetProduct()), nil
}

// SearchProductsAdvanced calls the remote microservice's GetProductsWithFilters (or similar).
func (r ProductRepository) SearchProductsAdvanced(
	ctx context.Context,
	name string,
	categoryId string,
	categorySlug string,
...
func (r ProductRepository) GetProductsByCategorySlug(
	ctx context.Context,
	categorySlug string,
	page, pageSize int64,
	sortBy, sortOrder string,
...
func (r ProductRepository) CreateProduct(ctx context.Context, name string, description string, basePrice int64, categoryID string, categorySlug string, brand string, condition string, model string, tags []string, manageStock bool, stock int64, sku string, attributes []models.Attribute, weight int64, height int64, width int64, depth int64, status string, negotiable bool, userType string, middlemanService bool, shippingCost int64, hasVariants bool, options []models.Option, lat float64, lng float64, thumbnail string, entityType models.EntityType) error {
...
func (r ProductRepository) GetUserProductCatalog(ctx context.Context, userID string, page int64, pageSize int64, sortBy string, sortOrder string) ([]*models.Product, error) {
...
func (r ProductRepository) GetPublicProductCatalog(ctx context.Context, userID string, page int64, pageSize int64, sortBy string, sortOrder string) ([]*models.Product, error) {
...
func (r ProductRepository) UpdateProductPriceForUser(ctx context.Context, productID string, newPrice int64, oldPrice int64) error {
...
// AdjustProductInventory adjusts product stock
func (r ProductRepository) AdjustProductInventory(ctx context.Context, productID string, newStock int64) error {
	log.Printf("[PRODUCT_GRPC] AdjustProductInventory called for productID=%s", productID)
...
// ArchiveUserProduct archives a product
func (r ProductRepository) ArchiveUserProduct(ctx context.Context, userID string, productID string) error {
	log.Printf("[PRODUCT_GRPC] ArchiveUserProduct called for productID=%s", productID)
...
// MarkProductAsSold marks a product as sold
func (r ProductRepository) MarkProductAsSold(ctx context.Context, productID string) error {
	log.Printf("[PRODUCT_GRPC] MarkProductAsSold called for productID=%s", productID)
...
// MarkProductAsLeased marks a product as leased
func (r ProductRepository) MarkProductAsLeased(ctx context.Context, productID string, monthlyPrice int64, leaseTermMonths int64) error {
	log.Printf("[PRODUCT_GRPC] MarkProductAsLeased called for productID=%s", productID)
...
// MarkProductAsPawned marks a product as pawned
func (r ProductRepository) MarkProductAsPawned(ctx context.Context, userID string, productID string, lockedPrice int64, redemptionFee int64) error {
	log.Printf("[PRODUCT_GRPC] MarkProductAsPawned called for productID=%s", productID)
...
// IncreaseProductPriceBy increases product price
func (r ProductRepository) IncreaseProductPriceBy(ctx context.Context, productID string, increaseAmount int64) error {
	log.Printf("[PRODUCT_GRPC] IncreaseProductPriceBy called for productID=%s", productID)
...
// DecreaseProductPriceTo decreases product price
func (r ProductRepository) DecreaseProductPriceTo(ctx context.Context, productID string, newPrice int64) error {
	log.Printf("[PRODUCT_GRPC] DecreaseProductPriceTo called for productID=%s", productID)
...
// DecreaseProductPriceTo decreases product price
func (r ProductRepository) UpdateProductPrice(ctx context.Context, productID string, newPrice int64) error {
	log.Printf("[PRODUCT_GRPC] DecreaseProductPriceTo called for productID=%s", productID)
...
// AddThumbnailToProduct adds a thumbnail to a product
func (r ProductRepository) AddThumbnailToProduct(ctx context.Context, productID string, thumbnail string) error {
	log.Printf("[PRODUCT_GRPC] AddThumbnailToProduct called for productID=%s", productID)
