...
// FindSimilarEntitiesByVectorMatch finds entities similar to a given entity
func (r *VectorRepository) FindSimilarEntitiesByVectorMatch(ctx context.Context, entityID string, entityType string, options domain.SearchSimilarOptions) (*domain.VectorSearchResults, error) {
	startTime := time.Now()
...
// PerformSemanticSearchWithVector performs semantic search using raw vector
func (r *VectorRepository) PerformSemanticSearchWithVector(ctx context.Context, vector []float32, options domain.VectorSearchOptions) (*domain.VectorSearchResults, error) {
	startTime := time.Now()
...
// GetPersonalizedRecommendationsForUser provides personalized recommendations
func (r *VectorRepository) GetPersonalizedRecommendationsForUser(ctx context.Context, userVector []float32, options domain.RecommendationOptions) (*domain.VectorSearchResults, error) {
	startTime := time.Now()
...
// RetrieveEntityContextualInformation retrieves contextual information about an entity
func (r *VectorRepository) RetrieveEntityContextualInformation(ctx context.Context, entityID string, entityType string, options domain.ContextOptions) (*domain.EntityContext, error) {
	if !r.circuitBreaker.CanExecute() {
...
// CheckVectorServiceHealthStatus checks if vector service is available
func (r *VectorRepository) CheckVectorServiceHealthStatus(ctx context.Context) bool {
	if !r.circuitBreaker.CanExecute() {
		return false
	}
