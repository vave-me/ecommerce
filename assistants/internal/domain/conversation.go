...
const ConversationAggregate = "assistants.Conversation"
...
func (c *Conversation) CreateConversation(id, userID, assistantID string, initialContext map[string]interface{}) (ddd.Event, error) {
	fmt.Printf("[CreateConversation] Called with: id=%s, userID=%s, assistantID=%s\n", id, userID, assistantID)
	...
	fmt.Printf("[CreateConversation] Event payload: ConversationID=%s, UserID=%s, AssistantID=%s\n",
		eventPayload.ConversationID, eventPayload.UserID, eventPayload.AssistantID)
...
func (c *Conversation) AddMessage(conversationsID, assistantID, messageID string, role MessageRole, content string, metadata map[string]interface{}, actionsTaken []AssistantAction) (ddd.Event, error) {
	if content == "" {
...
func (c *Conversation) ApplyEvent(event ddd.Event) error {
	...
	case *ConversationCreated:
		...
		c.AssistantID = payload.AssistantID
...
func (c *Conversation) ApplySnapshot(snapshot es.Snapshot) error {
	if conversationSnapshot, ok := snapshot.(ConversationV1); ok {
		...
		c.AssistantID = conversationSnapshot.AssistantID
...
type ConversationStats struct {
	...
	MostUsedAssistantID        string    `json:"most_used_assistant_id"`
}
