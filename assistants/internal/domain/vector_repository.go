...
type VectorRepository interface {
	// FindSimilarEntitiesByVectorMatch finds entities similar to a given entity using vector similarity
	// Returns empty results if vector service is unavailable
	FindSimilarEntitiesByVectorMatch(ctx context.Context, entityID string, entityType string, options SearchSimilarOptions) (*VectorSearchResults, error)

	// PerformSemanticSearchWithVector performs semantic search using raw vector embeddings
	// Returns empty results if vector service is unavailable
	PerformSemanticSearchWithVector(ctx context.Context, vector []float32, options VectorSearchOptions) (*VectorSearchResults, error)

	// GetPersonalizedRecommendationsForUser provides personalized recommendations based on user vector
	// Returns empty results if vector service is unavailable
	GetPersonalizedRecommendationsForUser(ctx context.Context, userVector []float32, options RecommendationOptions) (*VectorSearchResults, error)
	...
	RetrieveEntityContextualInformation(ctx context.Context, entityID string, entityType string, options ContextOptions) (*EntityContext, error)

	// CheckVectorServiceHealthStatus checks if vector service is available
	CheckVectorServiceHealthStatus(ctx context.Context) bool
}
