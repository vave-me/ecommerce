...
func TestCatalogRepository_UpdateConfigurationWithCapabilities(t *testing.T) {
	...
	repo := postgres.NewCatalogRepository("assistants", db)
	...
	// Set up expectation
	mock.ExpectExec("UPDATE assistants SET").
		WithArgs(
			assistantID,
			temperature,
			maxTokens,
...
func TestCatalogRepository_UpdateConfigurationWithCapabilities_DBError(t *testing.T) {
	...
	// Assert
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to update assistant configuration with capabilities")
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestCatalogRepository_Add_WithDuplicateCapabilities(t *testing.T) {
	...
	// Set up expectation
	mock.ExpectExec("INSERT INTO assistants").
		WithArgs(
			"test-id",
			"Test Assistant",
			"Test Description",
...
func TestCatalogRepository_FindAll_WithCapabilities(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		"id", "assistant_name", "description", "user_id", "type",
		...
		AddRow(
			...
			"user-123",
			"admin",
			pq.StringArray{"user_interaction", "data_analysis", "authentication", "assistant_management"},
			true,
			0.8,
			2000,
			"Prompt 2",
	...
	mock.ExpectQuery("SELECT .* FROM assistants").
		WillReturnRows(rows)

	// Execute
	assistants, err := repo.FindAll(ctx, "user-123")
	...
	assert.Equal(t, "assistant-1", assistants[0].ID)
	assert.Len(t, assistants[0].Capabilities, 2)
	assert.Contains(t, assistants[0].Capabilities, domain.CapabilityUserInteraction)
	assert.Contains(t, assistants[0].Capabilities, domain.CapabilityDataAnalysis)

	// Check second assistant
	assert.Equal(t, "assistant-2", assistants[1].ID)
	assert.Len(t, assistants[1].Capabilities, 4)
	assert.Contains(t, assistants[1].Capabilities, domain.CapabilityUserInteraction)
	assert.Contains(t, assistants[1].Capabilities, domain.CapabilityDataAnalysis)
	assert.Contains(t, assistants[1].Capabilities, domain.CapabilityAuthentication)
	assert.Contains(t, assistants[1].Capabilities, domain.CapabilityAssistantManagement)
