...
// AIClientProvider provides access to different AI clients
...
// LLMProcessor handles LLM integration for assistants
type LLMProcessor interface {
	ProcessWithHistory(ctx context.Context, assistant *domain.Assistant, currentMessage string, history []domain.ConversationMessage, contextData map[string]interface{}) (string, []domain.AssistantAction, float64, error)
	ShouldUseAI(assistant interface{}) bool
}
