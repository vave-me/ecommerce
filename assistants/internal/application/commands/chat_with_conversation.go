...
func (h ChatWithConversationHandler) ChatWithConversation(ctx context.Context, cmd ChatWithConversation) (*ChatWithConversationResult, error) {
	...
	// Get assistant ID - prefer from command, fallback to conversation
	assistantID := cmd.AssistantID
	if assistantID == "" {
		assistantID = conversation.AssistantID
	}
	if assistantID == "" {
		return nil, errors.Wrap(errors.ErrNotFound, "no assistant ID provided and conversation has no assistant ID")
	}
	...
	structuredData, naturalLanguageResponse := h.processActionsForDisplay(actions, response, cmd.Message, assistantID)
...
func (h ChatWithConversationHandler) buildEnhancedContext(conversation *domain.Conversation, cmd ChatWithConversation, history []domain.ConversationMessage) map[string]interface{} {
	...
	enhancedContext["assistant_id"] = conversation.AssistantID
...
func (h ChatWithConversationHandler) buildUpdatedContext(baseContext map[string]interface{}, history []domain.ConversationMessage, response string, actions []domain.AssistantAction, confidence float64) map[string]interface{} {
	...
	updatedContext["last_assistant_response_length"] = len(response)
...
func (h ChatWithConversationHandler) processActionsForDisplay(actions []domain.AssistantAction, response, userMessage, assistantID string) (map[string]interface{}, string) {
