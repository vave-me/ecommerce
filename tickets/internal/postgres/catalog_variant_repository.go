...
func (r CatalogVariantRepository) FindVariant(ctx context.Context, variantID string) (*domain.CatalogVariant, error) {
	...
	cv.Status = domain.ToTicketStatus(statusStr)
	cv.Condition = domain.ToTicketCondition(condStr)
...
func (r CatalogVariantRepository) GetVariant(ctx context.Context, variantID string) (*domain.CatalogVariant, error) {
	...
	return &domain.CatalogVariant{
		...
		TicketID:     catalogVariant.TicketID,
...
func (r CatalogVariantRepository) GetVariants(
	...
	countQ := fmt.Sprintf(`SELECT COUNT(*) FROM %s WHERE ticket_id = $1`, r.tableName)
	var totalCount int64
	if err := r.db.QueryRowContext(ctx, countQ, ticketID).Scan(&totalCount); err != nil {
	...
	rows, err := r.db.QueryContext(ctx, q, ticketID, pageSize, offset)
	if err != nil {
...
// -----------------------------------------------------------------------------
// 6) GetVariantsByTicket
// -----------------------------------------------------------------------------
func (r CatalogVariantRepository) GetVariantsByTicket(
	ctx context.Context,
	ticketID string,
	page, pageSize int64,
	sortBy, sortOrder string,
	...
	var totalCount int64
	if err := r.db.QueryRowContext(ctx, countQ, ticketID).Scan(&totalCount); err != nil {
		return nil, 0, errors.Wrap(err, "counting variants by ticket")
	}
	...
	rows, err := r.db.QueryContext(ctx, q, ticketID, pageSize, offset)
	if err != nil {
		return nil, 0, errors.Wrap(err, "retrieving variants by ticket")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, 0, errors.Wrap(err, "scanning variant by ticket row")
		}
		cv.Status = domain.ToTicketStatus(statusStr)
		cv.Condition = domain.ToTicketCondition(condStr)
		cv.Attributes = stringToAttributes(attrStr)
	...
	if err := rows.Err(); err != nil {
		return nil, 0, errors.Wrap(err, "finishing variant rows by ticket")
	}
	return results, totalCount, nil
}
...
func (r CatalogVariantRepository) GetVariantsByCategory(
	...
	// or doing a JOIN with tickets. We'll just return an error for now.
	return nil, 0, errors.Wrap(errors.ErrBadRequest,
		"Not Implemented: GetVariantsByCategory requires a join on the tickets table or storing category_id in variants")
}
