...
// Add provides a mock function with given fields: ctx, userID, email, username, location, enabled
func (_m *MockUserCacheRepository) Add(ctx context.Context, userID string, email string, username string, location string, enabled bool) error {
	ret := _m.Called(ctx, userID, email, username, location, enabled)
	...
	if rf, ok := ret.Get(0).(func(context.Context, string, string, string, string, bool) error); ok {
		r0 = rf(ctx, userID, email, username, location, enabled)
	} else {
		r0 = ret.Error(0)
	}
