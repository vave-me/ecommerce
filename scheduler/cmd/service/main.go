...
import (
	...
	"middleman/scheduler"
	"middleman/scheduler/migrations"
	"net/http"
)
...
func run(logger *logrus.Logger) (err error) {
	...
	schedulerCfg, err = config.InitSchedulerConfig()

	// Initialize system
	publicMethods := []string{}
	s, err := system.NewSchedulerSystem(cfg, publicMethods, schedulerCfg)
	if err != nil {
	...
	if err := scheduler.Root(s.Waiter().Context(), s); err != nil {
		logger.WithError(err).Error("Failed to initialize scheduler module")
		return err
	}

	logger.Info("Scheduler service started")
	defer logger.Info("Scheduler service stopped")
